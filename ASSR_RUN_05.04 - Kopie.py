...
for trial_data in trials:
    ...
    trial_start_psy = psychopy_clock.getTime()
    ...
    # exact trial end in device clock
    target_end_dev = trial_start_dev + SOA
    ...
    while True:
        ...
        # ---------------- trigger
        if resp_trig_frames > 0:
            draw_pixel(win, trigger_to_RGB(TRIG_RESPONSE))
            resp_trig_frames -= 1

        elif frameN < TRIG_FRAMES:
            draw_pixel(win, trigger_to_RGB(trigger_to_send))

        # ---------------- exact onset
        if frameN == 0:

            win.callOnFlip(audio_reg.play) # audio for psychopy
            # win.callOnFlip(device.audio.startSchedule) # audio for device
        ...
        # exact SOA lock (device clock)
        if device.getTime() >= target_end_dev:
            break
    ...
    print(
        "trial dur dev:",
        device.getTime() - trial_start_dev,
        "trial dur psy:",
        psychopy_clock.getTime() - trial_start_psy
    )
